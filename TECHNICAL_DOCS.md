...
- **Mechanism:**
  - Uses standard Arduino `pinMode()` in the `setup()` function to initialize the physical Start button (configured with `INPUT_PULLDOWN`) and status LEDs.
...
- **Language:** C/C++ Function Block.
- **Role:** Acts as the core Modbus state machine. It is a completely self-contained "black box" executing the programming cycle in three phases.
- **Phases:**
